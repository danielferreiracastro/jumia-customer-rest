...
public class CustomerServiceIntegrationUgandaTest extends AbstracBaseClass {
	...
	@Test
	public void getAllCustomersFilteredByCountryUganda() throws Exception {
		String url = "http://localhost:8080/v1/customers?country=256";
	...
	@Test
	public void getAllCustomersFilteredByCountryUgandaNotValid() throws Exception {
		String url = "http://localhost:8080/v1/customers?country=256&state=0";
		MvcResult mvcResult = mvc.perform(MockMvcRequestBuilders.get(url).accept(MediaType.APPLICATION_JSON_VALUE))
				.andReturn();
		String linkText = null;
		Map<String, Integer> pageMap = new HashMap<String, Integer>();
		...
		assertEquals(HttpStatus.OK.value(), status);
		assertEquals(10, pageMap.get("size").intValue());
		assertEquals(4, pageMap.get("totalElements").intValue());
		assertEquals(1, pageMap.get("totalPages").intValue());
		assertEquals(0, pageMap.get("number").intValue());
		assertEquals(1, numberOfLinks);
	}
	...
	@Test
	public void getAllCustomersFilteredByCountryUgandaValid() throws Exception {
		String url = "http://localhost:8080/v1/customers?country=256&state=1";
		MvcResult mvcResult = mvc.perform(MockMvcRequestBuilders.get(url).accept(MediaType.APPLICATION_JSON_VALUE))
				.andReturn();
		String linkText = null;
		Map<String, Integer> pageMap = new HashMap<String, Integer>();
